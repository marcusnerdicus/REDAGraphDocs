...
The Listing Map uses a Google Maps framework to display Active, Pending, and Closed listings on the MLS Region used by an Association and their Realtor's. The Filter panel allows a user to select different filter criteria to change the pins displayed on the map. the Blue button at the top of the Filter panel is the "Refresh" button and will turn orange when the filters have changed. The data in the Analytics section are recalculated each time the map is refreshed.

The Analyics panel calculations displayed use the current List Price for Active and Pending listings and the Closed price for Closed listings. When MLS data is loaded each day, all Active and Pending listings are included but there are only 13 months of Closed listings in the data set. Any Closed listing older than 13 months from the current date is ignored and not displayed on the map or available in reports. 
